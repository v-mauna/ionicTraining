...
describe('weather service', () => {
  ...
  afterEach(() =>
    (Plugins.Geolocation.getCurrentPosition as any).mockRestore(),
  );
  it('exists', () => {
  ...
  describe('current weather', () => {
    ...
    it('fetches the current weather for the current location', async () => {
      ...
      expect(window.fetch).toHaveBeenCalledWith(
        'https://api.openweathermap.org/data/2.5/weather?lat=42.123&lon=-73.4242&appid=0dd2e08fd19801e6177193ba30a52199',
      );
    });
  ...
  describe('forecast', () => {
    ...
    it('fetches the forecast for the current location', async () => {
      ...
      expect(window.fetch).toHaveBeenCalledTimes(1);
      expect(window.fetch).toHaveBeenCalledWith(
        'https://api.openweathermap.org/data/2.5/forecast?lat=42.123&lon=-73.4242&appid=0dd2e08fd19801e6177193ba30a52199',
      );
    });
...
describe('UV index', () => {
  ...
  it('fetches the UV Index for the current location', async () => {
    ...
    expect(window.fetch).toHaveBeenCalledTimes(1);
    expect(window.fetch).toHaveBeenCalledWith(
      'https://api.openweathermap.org/data/2.5/uvi?lat=42.123&lon=-73.4242&appid=0dd2e08fd19801e6177193ba30a52199',
    );
  });
